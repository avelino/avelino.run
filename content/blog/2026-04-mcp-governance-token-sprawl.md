...
tags = ["engineering leadership", "security", "mcp", "open source"]
description = "MCP made my team more productive. It also created 200 active tokens scattered across personal machines with no inventory, no rotation, and no central revocation. What we did about it."
...
**MCP as a protocol** is well thought out _(this also applies to the **CLI**)_. The problem is it was designed for individual use. When you scale to a team, the attack surface grows proportionally — number of engineers times number of services. With 20 engineers and 10 MCP servers, you potentially have 200 active tokens scattered across personal machines. No central audit. No visibility into who called what, when, with which arguments.
...
The solution we landed on was centralizing through a proxy. [`mcp serve`](https://mcp.avelino.run/guides/proxy-mode) exposes all configured servers as a single MCP endpoint. The team connects to the proxy — not directly to the services. The [token lives in one place](https://mcp.avelino.run/guides/proxy-mode#team-setup). Rotation becomes a single operation, not twenty. When someone leaves, you revoke one credential.
...
The proxy also solves a second problem we only noticed later: every MCP client — Claude Code, Cursor, any of them — spawns all configured servers at startup and keeps them alive forever. With 10 servers and 3 open sessions, you have 30 idle processes eating memory. The proxy fixes this with [lazy initialization and adaptive idle shutdown](https://mcp.avelino.run/mcp-servers-are-draining-your-hardware#lazy-initialization): backends only start when called, and shut down automatically when idle.
...
[Granular per-user permissions](https://mcp.avelino.run/guides/proxy-mode#access-control-acl) exist, but they're based on glob patterns over tool names — not semantic roles. You can say "engineer A gets no access to Slack tools", but not "engineer A can read but not write". It's a start, not mature access control.

The [audit log exists](https://mcp.avelino.run/guides/audit-logging) and is local by default. For real compliance, you'll want to integrate it with your observability stack. That's work, not magic.
